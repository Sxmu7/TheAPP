      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: '1.5rem' }}>
        {players.map((p) => (
          <span
            key={p.id}
            className="pill"
            style={{ background: p.color.fill, border: `1px solid ${p.color.stroke}`, color: p.color.text }}
          >
            {p.name}
          </span>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {GAMES.map((g) => (
          ...
          >
            ...
            <p style={{ fontWeight: 500, fontSize: 14, marginBottom: 4 }}>{g.name}</p>
            <p style={{ fontSize: 12, color: 'var(--text-2)', marginBottom: 10 }}>{g.desc}</p>
            <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 'var(--r-full)', background: 'var(--bg-2)', color: 'var(--text-3)' }}>
